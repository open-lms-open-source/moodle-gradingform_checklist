...
final class provider_test extends provider_testcase {
    ...
    public function test_export_gradingform_instance_data(): void {
        ...
        $DB->insert_record('gradingform_checklist_obs', [
            ...
            'observationdate' => make_timestamp(2026, 8, 2, 14, 30),
            'observationmode' => 'datetime',
        ]);
        ...
        $observation = writer::with_context($context)->get_data(['Test', 'Checklist', $instance->get_id(),
            get_string('observationdate', 'gradingform_checklist')]);
        $this->assertEquals(make_timestamp(2026, 8, 2, 14, 30), $observation->observationdate);
        $this->assertEquals('datetime', $observation->observationmode);
    }
    ...
    public function test_delete_gradingform_for_instances(): void {
        ...
        $DB->insert_record('gradingform_checklist_obs', [
            'instanceid' => $first['instance']->get_id(),
            'observationdate' => make_timestamp(2026, 8, 1),
            'observationmode' => 'date',
        ]);
        $DB->insert_record('gradingform_checklist_obs', [
            'instanceid' => $second['instance']->get_id(),
            'observationdate' => make_timestamp(2026, 8, 2),
            'observationmode' => 'date',
        ]);
        ...
        $this->assertTrue($DB->record_exists('gradingform_checklist_obs', ['instanceid' => $first['instance']->get_id()]));
        $this->assertFalse($DB->record_exists('gradingform_checklist_obs', ['instanceid' => $second['instance']->get_id()]));
    }
