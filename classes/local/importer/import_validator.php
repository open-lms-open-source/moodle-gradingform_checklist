...
class import_validator {
    ...
    public static function validate(array $raw, bool $allowfiles = false): canonical_import_data {
        ...
        if (!empty($raw['benchmark']) && is_array($raw['benchmark'])) {
            ...
            $data['benchmark']['buttonicon'] = self::clean_plain(
                $raw['benchmark']['buttonicon'] ?? 'fa-solid fa-file-circle-check'
            );
            ...
            if ($data['benchmark']['buttonicon'] === '') {
                $data['benchmark']['buttonicon'] = 'fa-solid fa-file-circle-check';
            }
            ...
            foreach ($raw['groups'] as $groupindex => $group) {
                ...
                if (!empty($group['items']) && is_array($group['items'])) {
                    foreach ($group['items'] as $itemindex => $item) {
                        ...
                        if (\core_text::strlen($definition) > \MoodleQuickForm_checklisteditor::ITEM_DEFINITION_MAX_LENGTH) {
                ...
                if (\core_text::strlen($description) > \MoodleQuickForm_checklisteditor::GROUP_DESCRIPTION_MAX_LENGTH) {
